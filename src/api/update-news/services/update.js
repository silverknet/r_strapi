...
function toNewsData(post, tag, pageId) {
  ...
  const parsedMessage = extractNewsContentFromMessage(post?.message, tag, null);

  if (!facebookPostId || !parsedMessage) {
    return null;
  }
  ...
  if (parsedMessage.date) {
    newsData.Datum = parsedMessage.date;
  }

  if (parsedMessage.place) {
    newsData.Plats = parsedMessage.place;
  }

  if (createdTime) {
    newsData.publishedAt = createdTime;
  }
